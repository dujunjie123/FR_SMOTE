...
class FruchtermannReingold:
    ...
    def algorithm_step(self, border_0, border_1):
        ...
        for i in range(len(self.vertices)):
            ...
            for j in range(i + 1, len(self.vertices)):
                vertex_2 = j
                #判断这两个点和各个特征边界的关系
                status = tool.position_with_border(self.positions[vertex_1], self.positions[vertex_2], border_0, border_1)
                status1 = 1
                status2 = 1
                ...
                self.forces[vertex_1] = self.sum(self.forces[vertex_1], \
                    self.mult(self.div(delta, mod_delta), status2*self.calculate_repulsion_force(mod_delta))
                )
                #在力的作用下，两个向量的移动方向是相反的
                self.forces[vertex_2] = self.sub(self.forces[vertex_2], \
                    self.mult(self.div(delta, mod_delta), \
                    status2*self.calculate_repulsion_force(mod_delta))
                )
        ...
        for edge in self.edges:
            ...
            self.forces[edge[0]] = self.sub(self.forces[edge[0]], 
                self.mult(self.div(delta, mod_delta), 
                status1*self.calculate_attraction_force(mod_delta))
            )
    ...
    def __draw__(self):
        values = np.array([v for v in self.positions.values()]).reshape(100, 2)
        tool.draw(values)
        return values
    ...
    def draw(self):
        ...
        for i in range(0, self.iterations):
            ...
            for key in self.positions:
                position_list.append(self.positions[key])
            feature1, feature2 = position_list
            ...
            self.algorithm_step(feature1_border, feature2_border)
            # self.__draw__()
            # time.sleep(1)
        value = self.__draw__()
        return value
